...
public class ScaleAnimation : MonoBehaviour
{
    ...
    {
        if (onAnim) return;
        _ref = StartCoroutine(ScaleAnim(AnimDelay, AnimTime, transform.localScale, AnimCurve));
    }

    public IEnumerator ScaleAnim(float delay, float time, Vector3 initScale, AnimationCurve curve)
    {
        onAnim = true;
        transform.localScale = shouldSaveStartScale ? startScale : Vector3.one;
